...
def simxGetVisionSensorImageFast(clientID, sensorHandle, options, operationMode):
    '''
    Faster version of the official function using numpy
    '''

    c_resolution = (ct.c_int*2)()
    c_image  = ct.POINTER(ct.c_byte)()
    bytesPerPixel = 3
    if (options and 1) != 0:
        bytesPerPixel = 1
    ret = c_GetVisionSensorImage(clientID, sensorHandle, c_resolution, ct.byref(c_image), options, operationMode)
    
    image = None
    resolution = None
    if (ret == 0):
        resolution = [c_resolution[0], c_resolution[1]]
        array_size = resolution[0] * resolution[1] * bytesPerPixel
        image = np.array(np.ctypeslib.as_array(c_image, shape=(array_size,)), copy=True, dtype=np.uint8)
    return ret, resolution, image

def simxGetVisionSensorImageFastBench(clientID, sensorHandle, options, operationMode):
    '''
    Faster version of the official function using numpy
    '''
    ...
    if (ret == 0):
        ...
        image = np.array(np.ctypeslib.as_array(c_image, shape=(array_size,)), copy=True, dtype=np.uint8)
        t1 = time() - t0
    return ret, resolution, image, t1, t2

def simxGetVisionSensorImageOrigBench(clientID, sensorHandle, options, operationMode):
    '''
    Please have a look at the function description/documentation in the V-REP user manual
    '''
